...
public class MediaInfo {
    ...
    public MediaInfo(Uri uri, String name, String duration) {
        this.mType = Type.STREAM;
        this.mUri = uri;
        this.mTitle = name;
        this.mDuration = duration;
    }

    public void changeToLocalType(){
        this.mType = Type.LOCAL;
    }

    public void changeToDownloadType(){
    ...
    public static ArrayList<MediaInfo> createAllMediaInfo(){
        ...
        for(int i=0;i<paths.length;++i){
            ...
            //Log.d("MediaInfo", "Uri == " + path);
            MediaInfo mediaItem = new MediaInfo(path, names[i], durations[i]);
            res.add(mediaItem);
        }
        return res;
    }
